...
public class FailureDetectionService {
    ...
    public void detectFailure() {
        ...
        try {
            ...
            sendPingRequest(new PingRequest(clusterConfig.getNodeId(), randomMember.getNodeId()), randomMember);
            ...
            final PingRequest indirectPingRequest = new PingRequest(clusterConfig.getNodeId(), randomMember.getNodeId());
            Utils.addPendingMessages(indirectPingRequest, pendingMessages, memberList, clusterConfig.getRound());

            boolean success = false;
            for (Member edgeMember : edgeMembers) {
                try {
                    final HttpResponse<String> response = sendPingRequestIndirect(indirectPingRequest, edgeMember);
